...
window.addEventListener("message", function (event) {
    ...
    if (event.data.action == "display") {
        ...
        $.each(event.data.players, function (index, player) {
            $("#modal_give > .modal-content").append('Choisisez l\'id du joueur :  <button class="waves-effect waves-red btn-flat btn-near-player white-text" data-player="' + player.player + '">#' + player.player + '</button>');
        });
        ...
        $(".btn-near-player").click(function () {
            ...
            $.post("http://c_inventory/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("http://c_inventory/NUIFocusOff", JSON.stringify({}));
}
...
function inventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        if(item.name == "water" || item.name == "cola" || item.name == "sprite"|| item.name == "oasis"|| item.name == "fanta"|| item.name == "icetea"|| item.name == "beer" || item.name == "energy_drink" || item.name == "coffee" || item.name == "juice"){
            using_icon = 'local_drink';
        }else if(item.name == "bread" || item.name == "burger" || item.name == "hotdog" || item.name == "taco" || item.name == "burrito" || item.name == "sanwich"  || item.name == "donut1" || item.name == "donut2" || item.name == "pie" || item.name == "chips" || item.name == "hamburger" || item.name == "saucisson" || item.name == "pizza"){
            using_icon = 'restaurant_menu';
        }else if(item.name == "smoke" || item.name == "cigarette" || item.name == "cigarett"){
            using_icon = 'smoking_rooms';
        }else if(item.name == "key"){
            using_icon = 'vpn_key';
        }

        if(item.name == "water" || item.name == "cola" || item.name == "bread" || item.name == "burger" || item.name == "hotdog" || item.name == "taco" || item.name == "burrito" || item.name == "sanwich" || item.name == "beer" || item.name == "energy_drink" || item.name == "donut1" || item.name == "donut2" || item.name == "pie" || item.name == "coffee" || item.name == "juice" || item.name == "chips" || item.name == "hamburger" || item.name == "saucisson" || item.name == "pizza" || item.name == "sprite"|| item.name == "oasis"|| item.name == "fanta"|| item.name == "icetea"){
            itemtype = "item_food";
...
$(document).ready(function () {
    ...
    $('body').on('click','[data-action="use"]', function(){
        ...
        if (itemData.usable) {
            $.post("http://c_inventory/UseItem", JSON.stringify({
                item: itemData
            }));
        }
    ...
    $('body').on('click','[data-button]', function(){
        ...
        $.post("http://c_inventory/SetCurrentMenu", JSON.stringify({
            type: type_value
        }));
    ...
    $('body').on('click','.action [data-action="give"]', function(){
        ...
        if(itemData.type == "item_weapon"){
            $.post("http://c_inventory/GetNearPlayers", JSON.stringify({
                item: itemData
            }));
        }else{
    ...
    $('body').on('click','.action [data-action="use_police"]', function(){
        var index = $(this).attr('data-index');
        itemData = $('#item-' + index).data("item");
        $.post("http://c_inventory/UseMask", JSON.stringify({
            item: itemData,
            other: player
        }));
    });
    ...
    $('body').on('click','.action [data-action="give_police"]', function(){
        ...
        if(itemData.type == "item_weapon"){
            ...
            //   item: itemData
            //}));
			$.post("http://c_inventory/GiveItemPolice", JSON.stringify({
				item: itemData,
				other: player,
				name1: name1,
				name2: name2,
    ...
    $('body').on('click','#item-submit[data-action="give"]', function(){
        ...
        if(itemData.type == "item_key" || itemData.type == "item_keyhouse"){
            ...
            if(count > 0){
                ...
                $("#text_result").text("Veuillez saisir une quantité").show();
                return false;
            }
        }
    ...
    $('body').on('click','.action [data-action="inventory_to_trunk"]', function(){
        ...
        if (type === "trunk" && itemInventory === "main") {
            if(itemData.type == "item_weapon"){
                $.post("http://c_inventory/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: 1
                }));
            }else{
    ...
    $('body').on('click','.action [data-action="inventory_to_house"]', function(){
        ...
        if (type === "house" && itemInventory === "main") {
            if(itemData.type == "item_weapon"){
                $.post("http://c_inventory/PutIntoHouse", JSON.stringify({
                    item: itemData,
                    number: 1
                }));
            }else{
    ...
    $('body').on('click','[data-action="house_to_inventory"]', function(){
        ...
        if (type === "house" && itemInventory === "second") {
            if(itemData.type == "item_weapon"){
                $.post("http://c_inventory/TakeFromHouse", JSON.stringify({
                    item: itemData,
                    number: 1
                }));
            }else{
